...
def create_thai_satellite_file(file_number):
    """Создает файл с тайским контентом для персональных спутников"""
    
    content = f"""# ПЕРСОНАЛЬНЫЙ_СПУТНИК_{file_number:03d}_ТАЙСКИЙ

## 108 สาขาดาวเทียมส่วนบุคคล - ไฟล์ที่ {file_number}
...
def main():
    """Создает 108 файлов с тайским контентом"""
    ...
    print("Создание 108 файлов с тайским контентом...")
    
    for i in range(1, 109):  # 1-108
        filename = f"ПЕРСОНАЛЬНЫЙ_СПУТНИК_{i:03d}_ТАЙСКИЙ.md"
        filepath = os.path.join(categories_dir, filename)
        
        content = create_thai_satellite_file(i)
        
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        
        if i % 10 == 0:
            print(f"Создано файлов: {i}/108")
    
    print("✅ Все 108 файлов успешно созданы!")
    print(f"Файлы сохранены в папке: {categories_dir}/")
